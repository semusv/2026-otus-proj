...
- **Смена роли** — селектор в строке пользователя той же карточки (`PATCH /admin/users/{user_id}`).
  Права применяются сразу: clearances резолвятся из БД на каждом запросе, перелогин не нужен.
  Смена собственной роли запрещена (403) — защита от потери последнего админа; смена роли
  пишется в аудит (`admin.user_role_changed` со старой и новой ролью).
