...
> конфигурируемый каталог (`CORPUS_HOST_DIR`) и загрузка файлов через API/UI
> («положил файл → нажал Ingest»). Автозапуска прогона после загрузки НЕТ — явно, кнопкой.

**Deliverables:** миграция `ingest_files`, инкрементальная логика pipeline, `POST /admin/documents`, upload UI, обновление README (backend/infra/root) + docs/minikube-deployment.md.

Решения (зафиксировано при планировании):
- Парсим ВСЕ XML каждый прогон (дёшево, секунды) — полный `corpus_ids` для корректного
  фильтра REFERENCES; дорогая часть (эмбеддинги/LLM-concepts) — только для дельты;
- unchanged-файлы: дешёвый ре-MERGE детерминированной части графа (старые акты получают
  REFERENCES на новые), БЕЗ эмбеддингов и LLM; changed/new — полный путь; removed —
  зачистка актов (Qdrant delete_act + Neo4j DETACH DELETE + чистка осиротевших Concept/Topic);
- снапшот корпуса — таблица PG `ingest_files` (filename, sha256, ingested_at, act_ids,
  chunks_count); форс-полный прогон — `--full` CLI / `?full=true` API (смена чанкера/модели);
- загрузка ТОЛЬКО сохраняет XML в каталог корпуса (валидация: расширение .xml, лимит
  `APP_INGEST_MAX_UPLOAD_MB`, санитизация имени, аудит) — прогон запускается отдельно;
- монтирование корпуса в compose и corpus-pvc в helm переводятся на rw (нужно для upload);
- minikube: новые образы — через Docker Hub с новым тегом (vvsem/graphrag-*).
...
Решения (зафиксировано при планировании):
- **README — точка входа преподавателя**: быстрый старт «с нуля» (compose: `.env` → корпус →
  `up` → seed-users → ingestion → UI), таблица «Соответствие заданию (tasks.md)» со ссылками
  на артефакты (диаграммы, ADR, load-report, RBAC-папка Postman), явный скоуп
  (тема Advanced RAG → GraphRAG; мультимодальность из шапки задания в MVP не входит),
  карта репозитория по формату сдачи;
- **Видео-скрипт — архитектурный нарратив** (защита курса «Архитектор ИИ»): не продуктовое
  демо, а формула «решение → почему (trade-off) → доказательство в рантайме»: C4 L1→L2→L3,
  Control/Data Plane, агент изнутри (state machine + re-plan), Security-by-Design с живым
  RBAC-доказательством, трейс как отражение графа агента в Jaeger, GraphRAG-данные в Neo4j,
  цифры нагрузки, minikube. Отдельно — формулировка, закрывающая требование «логи vLLM»
  (движок сменяем одной переменной; dev — LM Studio, целевой — vLLM, метрики в Grafana);
- финальный прогон — полная пирамида: lint → unit → integration → postman → frontend-гейты.

Чек-лист выполнения:

**A. Гигиена репозитория**
- [x] tmp-файлы удалены, `tmp_*` в .gitignore; устаревшая `docs/HANDOFF-fixes.md` удалена
      (фиксы из неё применены ранее); bump образов `stage11.0` в compose закоммичен

**B. README финализация**
- [x] быстрый старт с нуля (compose): предпосылки, `.env`, корпус RusLawOD, up (+observability),
      `make seed-users`, smoke, ingestion, демо-доступ viewer/analyst/admin, входы UI/Swagger/
      Neo4j/Jaeger/Grafana; замечание про первый медленный чат (докачка HF-моделей)
- [x] матрица «Соответствие заданию (tasks.md)» + скоуп-абзац; карта репозитория;
      minikube-блок и работа с корпусом сохранены; таблица стека в infra/README актуализирована

**C. Видео-скрипт**
- [x] `docs/video-demo-script.md`: чек-лист подготовки к записи, поминутный сценарий 0:00–7:00,
      проверенные демо-вопросы, Cypher для Neo4j Browser, формулировка vLLM, запасные тезисы Q&A

**D. `.env.example`**
- [x] ревизия: только dev-плейсхолдеры (`dev_*_password`, `change_me`); реальных секретов в git
      нет — из секретных файлов коммичен только `infra/helm/graphrag/secrets.yaml.example`

**E. Финальный полный прогон (все гейты зелёные)**
- [x] `make lint`: ruff + mypy (61 файл) — 0 проблем
- [x] `make test-unit`: **122 passed**
- [x] `make test-integration`: **65 passed, 0 skipped** (найден и закрыт долг: E2E Jaeger-проба
      после этапа 10 била через упразднённый маршрут jaeger.localhost и молча пропускалась —
      переведена на прямой `127.0.0.1:16686`, трейс находится)
- [x] `make test-postman` (newman через Traefik): **69 запросов / 99 assertions / 0 fail** (~2м49с)
- [x] фронтенд: eslint + tsc + vitest **19 passed** + production build (230.9 kB → 71.7 kB gzip)

**F. Закрытие**
- [x] коммиты подшагами `stage-11(...)`: chore → docs → tests; тег `stage/11`

Уроки этапа (учесть далее):
- **«Молчаливый» skip интеграционного теста — не норма на финальном прогоне**: тест E2E-трейса
  полтора этапа пропускался из-за устаревшей после рефакторинга инфраструктуры пробы. Правило:
  на финальной приёмке перепроверять skips так же строго, как failures;
- при выносе сервисов из Traefik (этап 10) синхронно обновлять прямые зависимости:
  тесты/скрипты, ходившие через поддомены;
