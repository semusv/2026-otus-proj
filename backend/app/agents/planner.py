...
REPLAN_SYSTEM = (
    ...
    "контекста с первой попытки. Выбери, как добрать информацию, и ответь СТРОГО "
    "JSON-объектом ровно в этом формате (оба поля обязательны): "
    '{"tools": ["retrieve_vec", "expand_graph"], "query": "<уточнённый поисковый запрос>"}. '
    'Поле "tools" - непустой список из допустимых значений: "retrieve_vec" '
    "(векторный поиск по чанкам документов), \"expand_graph\" (расширение по графу "
    "знаний: связанные акты, понятия). Поле \"query\" - переписанный запрос строкой. "
